...
The algorithm for unique decoding of AG codes, named K, is from [LBO2014]_ and
[Lee2016]_.
...
channel::
    ...
    sage: sent_codeword = code.encode(message)
...
class EvaluationAGCodeUniqueDecoder(Decoder):
    ...
    The saved object is to be removed after the final example.
    """
    _decoder_type = {'always-succeed'}

    def __init__(self, code, Q=None, verbose=False):
        """
        Initialize.
        ...
        if Q is None:
            ...
            deg = 1
            for p in code.base_function_field().places(deg):
            ...
            if Q is None:  # if none, then take a nonrational place
                while Q is None:
                    deg += 1
                    Q = code.base_function_field().get_place(deg)
        elif Q in code._pls:
        ...
        Decoder.__init__(self, code, code.ambient_space(), connected_encoder_name='Systematic')

        if Q.degree() == 1:
            circuit = _EvaluationAGCodeDecoder_K(code._pls, code._G, Q, verbose=verbose)
        else:
            circuit = _EvaluationAGCodeDecoder_K_extension(code._pls, code._G, Q, verbose=verbose)

        self._decoder = circuit.decode
    ...
    def decode_to_message(self, received_vector, **kwargs):
        ...
        EXAMPLES::
            ...
            True
        """
        return self.connected_encoder().unencode(self._decoder(received_vector, **kwargs))
    ...
    def decode_to_code(self, received_vector, **kwargs):
        ...
        """
        return self._decoder(received_vector, **kwargs)
...
class DifferentialAGCodeUniqueDecoder(Decoder):
    ...
    def __init__(self, code, Q=None, verbose=False):
        ...
        if Q.degree() == 1:
            circuit = _DifferentialAGCodeDecoder_K(code._pls, code._G, Q, verbose=verbose)
        else:
            circuit = _DifferentialAGCodeDecoder_K_extension(code._pls, code._G, Q, verbose=verbose)
...
class _Decoder_K(object):
    ...
    def decode(self, received_vector, verbose=False,
        ...
        return vector(K, sum([message[i] * code_basis[i] for i in range(len(message_index))]))
...
class _Decoder_K_extension(object):
    ...
    def decode(self, received_vector, **kwargs):
        """
        Decode received vector to a codeword.
        ...
        ``decoder`` is now ready to receive vectors from the noisy channel::

            sage: tau = decoder.decoding_radius()
            sage: chan = channels.StaticErrorRateChannel(code.ambient_space(), tau)
            sage: message_space = decoder.message_space()
            sage: message = message_space.random_element()
            sage: sent_codeword = code.encode(message)
            sage: received_vector = chan(sent_codeword)
            sage: decoder.decode_to_message(received_vector) == message
            True
        """
        received_vector_ext = self.lift(received_vector)
        return self.pull_back(self.decoder_ext.decode(received_vector_ext, **kwargs))
